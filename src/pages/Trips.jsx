  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:'1.5rem',paddingBottom:'5rem'}}>
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        <h1 style={{margin:0,fontSize:'1.5rem',fontWeight:700,color:'var(--text)'}}>טיולים ויציאות</h1>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'0.75rem'}}>
        <div className="stat-card" style={{textAlign:'center',padding:'0.875rem'}}>
          ...
          <div style={{fontSize:'0.75rem',color:'var(--text-muted)'}}>טיולים</div>
        </div>
        <div className="stat-card" style={{textAlign:'center',padding:'0.875rem'}}>
          <div style={{fontSize:'1.4rem',marginBottom:'0.25rem'}}>☕</div>
          <div style={{fontSize:'1.25rem',fontWeight:700,color:'#f59e0b'}}>{outingCount}</div>
          <div style={{fontSize:'0.75rem',color:'var(--text-muted)'}}>יציאות</div>
        </div>
        <div className="stat-card" style={{textAlign:'center',padding:'0.875rem'}}>
          <div style={{fontSize:'1.4rem',marginBottom:'0.25rem'}}>💸</div>
          <div style={{fontSize:'1.25rem',fontWeight:700,color:'#4ade80'}}>₪{totalCost.toLocaleString()}</div>
          <div style={{fontSize:'0.75rem',color:'var(--text-muted)'}}>סה״כ</div>
        </div>
      </div>

      {/* List */}
      {display.length === 0 ? (
        <div className="page-card" style={{textAlign:'center',padding:'3rem',color:'var(--text-dim)'}}>
          ...
          {display.map((t, i) => (
            <div key={t.id} style={{display:'flex',alignItems:'center',gap:'0.75rem',padding:'0.75rem 1rem',borderBottom: i < display.length-1 ? '1px solid rgba(255,255,255,0.04)':'none'}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontSize:'0.875rem',fontWeight:600,color:'var(--text)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{t.name}</div>
                <div style={{display:'flex',gap:'0.5rem',marginTop:'0.15rem',flexWrap:'wrap'}}>
                  ...
                  {t.date_from && <span style={{fontSize:'0.7rem',color:'var(--text-muted)'}}>{t.date_from}{t.date_to && t.date_to !== t.date_from ? ` — ${t.date_to}` : ''}</span>}
                  {t.description && <span style={{fontSize:'0.7rem',color:'var(--text-dim)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',maxWidth:120}}>{t.description}</span>}
                </div>
              </div>
              ...
              <div style={{display:'flex',gap:'0.125rem',flexShrink:0}}>
                <button onClick={()=>openEdit(t)} style={{background:'none',border:'none',cursor:'pointer',color:'var(--text-muted)',padding:'0.3rem'}}><Edit2 size={13}/></button>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0.5rem'}}>
                {[['date_desc','תאריך (חדש→ישן)'],['date_asc','תאריך (ישן→חדש)'],['cost_desc','עלות (גבוה→נמוך)'],['name','שם (א-ב)']].map(([k,v])=>(
                  <button key={k} onClick={()=>setSortBy(k)}
                    style={{padding:'0.4rem',borderRadius:'0.5rem',fontSize:'0.75rem',cursor:'pointer',border:`1px solid ${sortBy===k?'rgba(108,99,255,0.5)':'rgba(255,255,255,0.08)'}`,background:sortBy===k?'rgba(108,99,255,0.2)':'rgba(255,255,255,0.03)',color:sortBy===k?'#a78bfa':'var(--text-sub)'}}>{v}</button>
                ))}
              </div>
            </div>
      ...
      <Modal open={modal} onClose={()=>setModal(false)} title={editing ? 'ערוך' : 'הוסף טיול / יציאה'}>
        <div style={{display:'flex',flexDirection:'column',gap:'1rem'}}>
            ...
            <div style={{display:'flex',gap:'0.5rem'}}>
              {[['trip','🏕️ טיול'],['outing','☕ יציאה']].map(([k,v])=>(
                <button key={k} onClick={()=>setForm({...form,type:k})}
                  style={{flex:1,padding:'0.5rem',borderRadius:'0.5rem',fontSize:'0.85rem',cursor:'pointer',border:`1px solid ${form.type===k?`${TYPE_COLORS[k]}60`:'rgba(255,255,255,0.08)'}`,background:form.type===k?`${TYPE_COLORS[k]}20`:'rgba(255,255,255,0.03)',color:form.type===k?TYPE_COLORS[k]:'var(--text-sub)'}}>{v}</button>
              ))}
            </div>
          </div>
          <div>
          ...
          <div>
            <label style={{fontSize:'0.8rem',color:'var(--text-sub)',display:'block',marginBottom:'0.375rem'}}>תיאור (לא חובה)</label>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0.75rem'}}>
            <div>
              <label style={{fontSize:'0.8rem',color:'var(--text-sub)',display:'block',marginBottom:'0.375rem'}}>מתאריך</label>
              <input className="input-field" type="date" value={form.date_from} onChange={e=>setForm({...form,date_from:e.target.value})} dir="ltr"/>
            </div>
            <div>
              <label style={{fontSize:'0.8rem',color:'var(--text-sub)',display:'block',marginBottom:'0.375rem'}}>עד תאריך</label>
              <input className="input-field" type="date" value={form.date_to} onChange={e=>setForm({...form,date_to:e.target.value})} dir="ltr"/>
            </div>
          </div>
          <div>
            <label style={{fontSize:'0.8rem',color:'var(--text-sub)',display:'block',marginBottom:'0.375rem'}}>עלות כוללת ₪ (לא חובה)</label>
